...
if monthly:                
  ...
  if monthfclm==12:
    print('processing december')
    datef=str(yearclm+1)+'-'+str(1).zfill(2)+'-'+str(1).zfill(2) + ' ' +str(0).zfill(2)+':'+str(0).zfill(2)+':'+str(0).zfill(2)
  else:
    datef=str(yearclm)+'-'+str(monthfclm+1).zfill(2)+'-'+str(1).zfill(2) + ' ' +str(0).zfill(2)+':'+str(0).zfill(2)+':'+str(0).zfill(2)
  ...
  x_fm=infofile['longitude'][:]

  y_fm=infofile['latitude'][:]
  y_fm=y_fm[::-1] 
  ...
  datei = str(yearclm)+'-'+str(monthiclm).zfill(2)+'-'+str(1).zfill(2) + ' ' +str(0).zfill(2)+':'+str(0).zfill(2)+':'+str(0).zfill(2) 
  bry_time[0]  = bry_time[0] - 1  + np.diff(dates.datestr2num([timeref,datei])) #since the timecount starts at day 1
  
  bry_time = bry_time * 24*60*60               #seconds since

  numdays=int(len(n_time))
  n_time = int(numdays)         #number of months
...
for rt in range(int(numdays)):
  ...
  if monthly:        
    print('########################## month = ' + str(rt+1) + ' #########################' )
    print(numdays, tempclm.shape)
    temp_fm = tempclm[rt,::]
    salt_fm = saltclm[rt,::]
    u_fm = uclm[rt,::]
    v_fm = vclm[rt,::]
