...
def horiz_interp_3dvar(variable_pgrd, lon_pgrd, lat_pgrd, z_pgrd, lon_cgrd, lat_cgrd, topo_cgrd):
	...
	variable_cgrd = np.ma.zeros( (Nn,)+topo_cgrd.shape )
	   ...
	   if ff.mean()==1.0:
              ...
              points = (lat_pgrd[~ff].ravel(), lon_pgrd[~ff].ravel())
              interp_points = (lat_cgrd.ravel(), lon_cgrd.ravel())
              ndir=griddata(points, pvar3d_slice[~ff].ravel(), interp_points, method='linear').reshape(x_roms.shape)
...
if 'f-plane' in fname_grd:
	...
	if 'sm_' in fname_grd:
		...
		run_name = 'prooceano_myocean_cortado_cuzao_666'
