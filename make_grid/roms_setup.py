...
def add_topo(lon, lat, pm, pn, toponame):
        ...
        # dat = loadmat(toponame)
        nc=netCDF4.Dataset(toponame)
        x   = nc['x'][:]
        y   = nc['y'][:]
        z   = nc['z'][:]
...
#        z  = z[0::d, 0::d]
        ...
        h = griddata((x.ravel(),y.ravel()),h.ravel(),(lon.ravel(),lat.ravel())).reshape(lat.shape)
...
def smoothgrid(hraw,maskr,hmin,hmax_coast,rmax,n_filter_deep_topo,n_filter_final):
        """
        #
